...
step = 1050 #milling step in um
segment = 4300 # lenght of 1 cutting segment
N_of_cuts = 0
...
while True:
    ...
    if txt == "EXIT":
        ...
        for seg in range(5):
            writemsg("M5")  # stop spindel
            ser.flushInput()
            txt = input(f"Make {seg+1} segment?:").upper()
            if txt == 'Y':
                step = int(input("enter step in um:"))
                N_of_cuts = segment // step
                print(N_of_cuts)
                writemsg("G90")
                writemsg("G55")
                writemsg(f"G1X{segm[seg][0]}Y{segm[seg][1]} F150")  #go to start of segment
                ...
                for i in range(N_of_cuts):
                    ...
                    if (txt == 'Y') and (seg == 0 or seg == 2 or seg == 4):
                        cut_South()
                        while stateresponde() == "RUN": time.sleep(1)
                        print(f"step {i}")
                    elif (txt == 'Y') and (seg == 1 or seg == 3):
                        cut_Nord()
                        while stateresponde() == "RUN": time.sleep(1)
                        print(f"step {i}")
                    else:
